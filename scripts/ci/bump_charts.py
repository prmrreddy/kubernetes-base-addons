...
def update_app_version(loaded_yaml, info, app_version):
    ...
    for k, v in loaded['metadata']['annotations'].items():
            if k.startswith('appversion') and update_app_version:
                loaded['metadata']['annotations'][k] = app_version
            elif k.startswith('catalog'):
                if update_app_version:
                    loaded['metadata']['annotations'][k] = app_version + '-1'
                else:
                    revision_number = 1
                    ...
                    if len(split_revision) != 0:
                        try:
                            ...
                            pass
                    loaded['metadata']['annotations'][k] = '{}-{}'.format('-'.join(split_revision[:-1]), revision_number)
                ...
                loaded['metadata']['annotations'][k] = v.replace(minor_current_app_version, minor_new_app_version)
                ...
                loaded['metadata']['annotations'][k] = v.replace(old_sha, new_sha)
